...
in S/R! To make things worse, the syntax of the two class types is different. Using S4 classes is different to the way you use common classes found in R packages.
...
```{r}
powerTool <- function(myX) {
    get <- function() myX
    set <- function(newX) myX <<- newX
    sqr <- function() pow(2)
    pow3 <- function() pow(3)
    pow <- function(y) myX ^ y
    instance <- list(
        get=get,
        set=set,
        sqr=sqr,
